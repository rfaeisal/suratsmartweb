      ...
      <div className="flex gap-1 mb-6 border-b border-gray-200 dark:border-slate-700">
        ...
        >
          ...
          <span className={`ml-2 px-1.5 py-0.5 text-xs rounded-full ${
            tab === "registered" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300" : "bg-gray-100 text-gray-500 dark:bg-slate-700 dark:text-slate-400"
          }`}>
            {users.length}
          </span>
        </button>
        ...
        >
          ...
          {neverLoggedIn.length > 0 && (
            <span className={`ml-2 px-1.5 py-0.5 text-xs rounded-full ${
              tab === "never" ? "bg-amber-100 text-amber-700 dark:bg-yellow-900/40 dark:text-yellow-300" : "bg-amber-50 text-amber-600 dark:bg-yellow-900/20 dark:text-yellow-400"
            }`}>
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            <div className="relative flex-1">
              ...
              />
              {query && (
                <button onClick={() => setQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300">
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500 mb-3">{filteredUsers.length} dari {users.length} pengguna</p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 divide-y divide-gray-100 dark:divide-slate-700">
            {filteredUsers.length === 0 ? (
              <div className="py-12 text-center">
                <svg className="mx-auto mb-3 text-gray-300 dark:text-slate-600" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
                </svg>
                <p className="text-sm text-gray-400 dark:text-slate-500">Tidak ada pengguna yang cocok</p>
                ...
                return (
                  <div key={user.id} className="px-5 py-4 flex items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 dark:text-slate-100">{user.employee.fullName}</p>
                      ...
                      <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
                        Username:{" "}
                        {user.username
                          ? <span className="font-mono text-gray-600 dark:text-slate-400">{user.username}</span>
                          : <span className="text-gray-300 dark:text-slate-600">—</span>}
                      </p>
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {user.roles.map((r) => (
                          <span key={r} className="inline-block px-2 py-0.5 text-xs font-medium rounded-full bg-blue-50 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                            {ROLE_LABELS[r] ?? r}
                          </span>
                        ))}
                      </div>
      ...
      {tab === "never" && (
        <>
          <div className="relative mb-5">
            ...
            />
            {queryNever && (
              <button onClick={() => setQueryNever("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                </svg>
              </button>
          ...
          <div className="mb-3 flex items-center gap-2 flex-wrap">
            <p className="text-xs text-gray-400 dark:text-slate-500">{filteredNever.length} dari {neverLoggedIn.length} pegawai</p>
            {neverLoggedIn.length > 0 && (
          ...
          {neverLoggedIn.length === 0 ? (
            <div className="py-16 text-center bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
              ...
              <p className="text-sm font-medium text-gray-700 dark:text-slate-300">Semua pegawai sudah pernah login</p>
              <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">Tidak ada pegawai aktif yang belum menggunakan CutiSmart</p>
            </div>
          ) : filteredNever.length === 0 ? (
            <div className="py-12 text-center bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700">
              <p className="text-sm text-gray-400 dark:text-slate-500">Tidak ada hasil yang cocok</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                  {filteredNever.map((emp) => (
                    <tr key={emp.id} className="hover:bg-amber-50/40 dark:hover:bg-slate-700/50 transition-colors">
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900 dark:text-slate-100">{emp.fullName}</p>
                        <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5 font-mono">{emp.nip}</p>
                      </td>
                      <td className="px-4 py-3 text-gray-600 dark:text-slate-400 text-xs">{emp.unit?.name ?? <span className="text-gray-300 dark:text-slate-600 italic">—</span>}</td>
                      <td className="px-4 py-3 text-gray-600 dark:text-slate-400 text-xs">{emp.positionTitle ?? <span className="text-gray-300 dark:text-slate-600 italic">—</span>}</td>
                      <td className="px-4 py-3">
