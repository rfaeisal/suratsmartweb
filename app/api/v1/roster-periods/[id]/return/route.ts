...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  let authUser
  try {
    authUser = await requireAuth(req, ["ADMIN_UNIT", "KEPALA_UNIT", "ADMIN_KEPEGAWAIAN", "SUPERADMIN"])
  } catch (e) {
  ...
  if (period.status !== "PENDING_APPROVAL") {
    return Errors.conflict(
      period.status === "DRAFT"
        ? "Roster masih dalam status draft"
        : "Tidak bisa mengembalikan roster yang sudah diterbitkan",
    )
  }
  ...
  if (isUnitRole && !isAdminRole && authUser.managedWorkUnitId !== period.workUnitId) {
